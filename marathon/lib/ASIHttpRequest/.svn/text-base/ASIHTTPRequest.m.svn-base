...
//- (void)cancel
//{	
//    // NSLog(@"ASIHTTP clear and Cancel called!");
//    [[self cancelledLock] lock];
//    
//	// Clear delegates
...
//
//    [self performSelector:@selector(cancelOnRequestThread) onThread:[[self class] threadForRequest:self] withObject:nil waitUntilDone:NO];    
////    [self performSelector:@selector(cancelOnRequestThread) onThread:[NSThread mainThread] withObject:nil waitUntilDone:NO];    
//}
...
- (void)reportFinished
{
    if (!isRequestCancelled) {
        ...
        // NSLog(@"isCanceled : %d, reportFinished", isRequestCancelled);
        if (delegate && [delegate respondsToSelector:didFinishSelector]) {
            // NSLog(@"delegate %@ :", delegate);
            [delegate performSelector:didFinishSelector withObject:self];
            // NSLog(@"delegate 1 %@ :", delegate);
        }
...
{
    if (!isRequestCancelled) {
        // NSLog(@"isCanceled : %d, reportFailure", isRequestCancelled);
