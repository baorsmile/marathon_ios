...
-(NSString*)filenameForOid:(id)oid {
    ...
    if ([oidStr length] == 0) {
        NSLog(@"HJMOFileCache failed to move loading file to ready file, error : %@", oid);
    }
    
//    if (![oidStr hasSuffix:@".jpg"] && ![oidStr hasSuffix:@".png"]) {
//        oidStr = [loadingPath stringByAppendingString:@".png"];
//    }
...
-(NSString*)loadingFinishedForOid:(id)oid {
	...
	if (e) {
         NSLog(@"HJMOFileCache failed to move loading file to ready file, error : %@", e);
