...
+ (void)initialize
{
    ...
    {
        background = [UIImage imageByName:kActionSheetBackground];
...
- (void)addButtonWithTitle:(NSString *)title Icon:(UIImage *)iconImage color:(NSString*)color block:(void (^)())block atIndex:(NSInteger)index
{
    if (index >= 0)
    {
...
- (void)addButtonWithTitle:(NSString *)title Icon:(UIImage*)iconImage block:(void (^)())block {
    [self addButtonWithTitle:title Icon:iconImage color:@"gray" block:block atIndex:-1];
}
...
- (void)showInView:(UIView *)view
{
    ...
    for (NSArray *block in _blocks)
    {
        ...
        if ([block count]>3) {
            iconImage = [block objectAtIndex:3];
        }
        
        
        UIImage *image = [UIImage imageByName:[NSString stringWithFormat:@"action-%@-button.png", color]];
